...
from quantools import FractionalDiff
...
def test_FractionalDiff(n_col: int = 10):
    ...
    diff = FractionalDiff()
    _, orders = diff(X, return_order=True, precision=1e-3)
    assert (
        all(np.array(orders) > 0)
        and len(orders) == n_col
        and len(np.unique(orders)) > 1
    )
